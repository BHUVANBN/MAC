...
static int compute_hmac(const unsigned char *mac_key, size_t mac_key_len,
    ...
    unsigned int len = 0;
    HMAC_CTX *h = HMAC_CTX_new(); // may warn on OpenSSL3 but works
    if (!h) return -1;
...
// frame: send 4-byte lengths then iv, ct, mac
...
// global keys
static unsigned char ENC_KEY[AES_KEYLEN];
static unsigned char MAC_KEY[HMAC_KEYLEN];

typedef struct { int sock; } thrarg_t;
